...
const Favorites = () => {
  ...
  const fetchFavorites = async () => {
    try {
      const data = await apiRequest('/user/favorites', 'GET');
      setFavorites(data);
    } catch (err) {
  ...
  const removeFavorite = async (githubId: number) => {
    try {
      await apiRequest(`/user/favorites/${githubId}`, 'DELETE');
      // Update local state to remove the item immediately
      setFavorites(favorites.filter(repo => repo.githubId !== githubId));
    } catch (err) {
